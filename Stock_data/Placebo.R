...
select(-c(daily_return, MR, val, total_val, PX_OPEN, AV_industy30, AV_industy20, AV_industy10)) # Remove variables used for calculations
...
stock_data <- stock_data %>% filter(Security != "SOFF") # Remove SOFF as this company had extreme values
...
earnings_calc(stock_data$eps, stock_data$ES_4d, stock_data$CAR1, stock_data$news_q)


## 6. Switch Values for AV/VOL
...
stock_data$news_q <- as.numeric(stock_data$news_q)
...
stargazer(mod1, mod2, mod3, mod4, 
          ...
          notes = "Table 7 shows the results of the multivariate regression on how the amount of Corona articles published by NRK effects abnormal trading volume of stocks publishing an earnings announcement on the same day. In this case Abnormal Volume is replaced by that of another company in the same industry in the same day. In regression (3) and (4) only the observations from the top two Cov-quantiles are used. Regression (2) and (4) adjust for control the variables Month, Market Capitalization Deciles, Market-to-Book ratio Deciles, IO share and Number of Analysts. All control variables are averaged for each month. Standard errors are adjusted for heteroskedasticity and t-values are reported in the parentheses. *, **, and *** represent significance at the 10%, 5% and 1% level, respectively.",
          notes.append = FALSE,
          notes.label = "",
          notes.align = "l",
          model.numbers = FALSE,
